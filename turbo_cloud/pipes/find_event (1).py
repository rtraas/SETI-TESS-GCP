...
def group_events(event_csv_file, h5_dir='/tess_container/seti_tess/l_band/'):
    """

    Grouping algorithm that assigns events into groups.

    Events should be grouped into frequency bins of 125 kHz.
    In each bin, the spacing between the highest and lowest start frequencies is computed,
        in order to compute an effective bandwidth 'event_bw' (delta_{nu_event}) and
        central frequency 'event_freq' (nu_{event}).

    We refer to each "cluster" as an "event group".
    ...
    event = pd.read_csv(event_csv_file)

    # First, add a 'kHz' column for each 'frequency' column
    event['Freq khz'] = event['Freq'].to_numpy()*1000.0
    event['FreqStart khz'] = event['FreqStart'].to_numpy()*1000.0
    event['FreqEnd khz'] = event['FreqEnd'].to_numpy()*1000.0

    # Second, get full frequency range spanned by the source file
    path_to_source = os.path.relpath(h5_dir, os.getcwd())
    source = bl.Waterfall(path_to_source+'/'+event['FileID'][0], load_data=False)
    source_minfreq = source.container.f_start
    source_maxfreq = source.container.f_stop - source.header['foff']

    # Third, bin the data into frequency bins of 125 kHz
    source_minfreq = round(source_minfreq, 6)
    source_maxfreq = round(source_maxfreq, 6)

    binwidth = 0.125
    num_bins = np.ceil((source_maxfreq - source_minfreq)/binwidth)
    bins = np.arange(source_minfreq, source_minfreq + (num_bins + 1) * binwidth, binwidth)
    bin_container = [[i, [], {}] for i in bins]

    for e in range(len(event)):
        print("Searching for a bin for event %i of %i"%(e+1, len(event)))
        for i in range(len(bin_container)):
            if (event.iloc[e]['FreqStart'] > bin_container[i][0]) and (event.iloc[e]['FreqStart'] < bin_container[i+1][0]):
                bin_container[i][1].append(event.iloc[e])
                break
            else:
                continue

    valid_bins = [b for b in bin_container if len(b[1])!=0]
    print("The events have been grouped into %i frequency bins of 125 kHz"%(len(valid_bins)))

    # Fourth, in each bin, compute the spacing between the highest and 
    ### lowest start frequencies
    for vb in valid_bins:
        vb_freqs = np.array([v['FreqStart'] for v in vb[1]])
        vb[2]['upperfreq'] = vb_freqs.max()
        vb[2]['lowerfreq'] = vb_freqs.min()
        vb[2]['spacing']   = vb_freqs.max()-vb_freqs.min()
        vb[2]['centralfreq']=vb_freqs.min() + (vb_freqs.max() - vb_freqs.min())/2.0
